...
{
    printf("usage:\r\n");
    printf("    mkz [-r reserve-image-size] [-k aes256-encrypt-key] [-pb ecdsa256-public-key] [-pv ecdsa256-private-key] [-m message] [-g] [-i] [-e] <image> <zimage>\r\n");
...
static int do_mkz(int argc, char ** argv)
{
    ...
    int index = 0, rsize = 0, keygen = 0, uniqueid = 0, encrypt = 0;
    int fd, bllen, zbllen;
    int clen, len;
    int i, o;
    ...
    if(keygen)
        machine_keygen(machine_uniqueid(), key);
    ...
    if(msg)
        strlcpy((char *)&z->message[0], msg, 80);
